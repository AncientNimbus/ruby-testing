...
describe FindNumber do
  ...
  describe '#make_guess' do
    ...
    # explained above.
    let(:number_guessing) { double('number_guess', value: 8) }
    ...
    context 'when min is 5 and max is 9' do
      subject(:min5_max9) { described_class.new(5, 9, number_guessing) }
      it 'returns 7' do
        expect(min5_max9.make_guess).to eq(7)
      end
    end

    context 'when min is 8 and max is 9' do
      subject(:min8_max9) { described_class.new(8, 9, number_guessing) }
      it 'returns 8' do
        expect(min8_max9.make_guess).to eq(8)
      end
    end

    context 'when min is 0 and max is 3' do
      subject(:min0_max3) { described_class.new(0, 3, number_guessing) }
      it 'returns 1' do
        expect(min0_max3.make_guess).to eq(1)
      end
    end

    context 'when min and max both equal 3' do
      subject(:min3_max3) { described_class.new(3, 3, number_guessing) }
      it 'returns 3' do
        expect(min3_max3.make_guess).to eq(3)
      end
    end
  end
  ...
  describe '#game_over?' do
    ...
    context 'when guess and random_number are equal' do
      ...
      subject(:guess_eq_random_number) { described_class.new(0, 9, random_number, 5) }
  ...
  describe '#update_range' do
    ...
    context 'when the guess is 7, min=5, and max=8' do
      subject(:min5_max8) { described_class.new(5, 8, number_range, 7) }

      it 'updates min to the same value as max' do
        min5_max8.update_range
        expect(min5_max8.min).to eq(min5_max8.max)
      end

      it 'does not update max' do
        min5_max8.update_range
        expect(min5_max8.max).to eq(8)
      end
    end
  end
end
