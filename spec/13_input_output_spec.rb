...
describe NumberGame do
  ...
  describe '#game_over?' do
    ...
    context 'when user guess is not correct' do
      subject(:game_not_over) { described_class.new(8, '2') }
  ...
  describe '#player_turn' do
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        ...
        big_number = '99'
        valid_input = '5'
        user_inputs = [letter, big_number, valid_input]
        allow(game_loop).to receive(:player_input).and_return(*user_inputs)
      end
  ...
  describe '#final_message' do
    ...
    context 'when count is 2-3' do
      subject(:game_under_three) { described_class.new(8, '8', 3) }
      it 'outputs correct phrase' do
        congrats_phrase = "Congratulations! You picked the random number in 3 guesses!\n"
        expect { game_under_three.final_message }.to output(congrats_phrase).to_stdout
      end
    end

    context 'when count is 4 and over' do
      subject(:game_over_three) { described_class.new(9, '9', 5) }
      it 'outputs correct phrase' do
        game_message = "That was hard. It took you 5 guesses!\n"
        expect { game_over_three.final_message }.to output(game_message).to_stdout
      end
    end
  end
end
