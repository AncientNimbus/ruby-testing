...
describe BinaryGame do
  ...
  describe '#verify_input' do
    ...
    subject(:check_input) { described_class.new(1, 20) }

    context 'when given a within bound input as argument' do
      it 'returns valid input' do
        min = check_input.instance_variable_get(:@minimum)
        max = check_input.instance_variable_get(:@maximum)
        valid_input = 18
        result = check_input.verify_input(min, max, valid_input)
        expect(result).to eq(18)
      end
    end

    context 'when given out of bound input as argument' do
      it 'returns nil' do
        min = check_input.instance_variable_get(:@minimum)
        max = check_input.instance_variable_get(:@maximum)
        invalid_input = 100
        result = check_input.verify_input(min, max, invalid_input)
  ...
  describe '#display_turn_order' do
    ...
    it 'increases guess_count by one' do
      expect { game_display.display_turn_order(search_turn) }.to change { game_display.instance_variable_get(:@guess_count) }.by(1)
    end

    # Method with Outgoing Command -> Test that a message is sent
    it 'sends make_guess' do
      expect(search_turn).to receive(:make_guess).once
      game_display.display_turn_order(search_turn)
    end
